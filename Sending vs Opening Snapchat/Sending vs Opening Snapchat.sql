...
*/
WITH CTE1 AS (
    SELECT * FROM activities AS a JOIN age_breakdown AS ab ON a.user_id = ab.user_id
WHERE a.activity_type IN ('open','send') ),

CTE2 AS (
    SELECT age_bucket,
    SUM(time_spent) FILTER (WHERE activity_type = 'open') AS open_sum,
    SUM(time_spent) FILTER (WHERE activity_type='send') AS send_sum
    FROM CTE1
    GROUP BY age_bucket )
    
SELECT age_bucket, ROUND(send_sum * 100.0/ (open_sum+send_sum),2) AS send_perc ,
ROUND(open_sum * 100.0 / (open_sum+send_sum),2) AS open_perc 
FROM CTE2
