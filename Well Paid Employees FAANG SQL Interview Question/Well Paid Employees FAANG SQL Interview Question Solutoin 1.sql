...
WITH CTE1 AS (SELECT e1.employee_id AS emp_id, e1.name AS emp_name,
e1.salary AS emp_salary,
e2.name AS man_name, e2.salary AS man_salary 
FROM employee e1
JOIN employee e2 on e1.manager_id = e2.employee_id)
SELECT emp_id, emp_name FROM CTE1 WHERE emp_salary > man_salary