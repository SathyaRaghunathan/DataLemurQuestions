...
DENSE_RANK() OVER (
PARTITION BY department_id ORDER BY salary DESC) AS rk
FROM employee  

)
SELECT d.department_name, a.name, a.salary FROM CTE1 AS a
JOIN department AS d on a.department_id = d.department_id
WHERE rk <=3
ORDER BY 1,3 DESC,2
